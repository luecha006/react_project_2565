...
export default function AddPage() {
  ...
  useEffect(() => {
    if (imageProfile.length < 1);
      ...
      imageProfile.forEach((image) =>
        newImageProfileURLs.push(URL.createObjectURL(image))
      );
    ...
    if (imageDisease1.length < 1);
      ...
      imageDisease1.forEach((image) =>
        newImagesDiseaseURL.push(URL.createObjectURL(image))
      );
  ...
  async function onUpload() {
    ...
    if (
      ...
      image8.length !== 0 &&
      image9.length !== 0 &&
      imageDisease1.length !== 0 &&
      imageDisease2.length !== 0 &&
      imageDisease3.length !== 0
    ) {
      setIsUpload(true);
      ...
      for (var i = 0; i < sumName.length; i++) {
        //อัพรูปเก็บ url
        await storageRef
          .child(`WebAddImages/${sumName[i]}`)
          .put(sumfile[i])
          .then(async (snapshot) => {
            await storageRef
              .child(`WebAddImages/${sumName[i]}`)
  ...
  return (
    <div className="addPageSpace bg container-fluid  mb-4  ">
      <div
        className="container"
        style={{ width: "80%", height: "100%", marginBottom: "4%" }}
      >
        <Toast ref={toast} />
        <br />
        <Card className="card-body">
          <Card.Body className="card-body">
            <h2>เพิ่มข้อมูลกระบองเพชร</h2>
            <br />
            <br />
            <div>
              <Form style={{ textAlign: "start" }}>
                ...
                <Row>
                  <Col className="head ">
                    <Form.Group controlId="formHead4">
                      <Form.Control
                        type="text"
                        placeholder="เพิ่มโรคเเละปัญหาที่พบบ่อย"
                ...
                <Row>
                  ...
                  <Col style={{ textAlign: "center" }}>
                    <Form.Control
                      ...
                      className="mt-2 mb-2"
                      type="file"
                      onChange={(e) => setImageDisease3([...e.target.files])}
                    />
                  </Col>
                  <Col xl lg xxl="3" style={{ textAlign: "center" }}></Col>
                </Row>
                ...
                <Row className="pb-8 pt-0">
                  <Col>
                    <Form.Group className="mb-3" controlId="refer1">
                      <Form.Control
                        type="text"
                        placeholder="เพิ่มที่มาของแหล่งข้อมูล"
                ...
                <div style={{ textAlign: "center" }}>
                  <Button
                    className="btn-success  p-2 px-4 p-button-info"
                    style={{ width: "25%" }}
                    disabled={isUpload}
                    onClick={onUpload}
                  >
